  ...
  var laxInstance = function () {
    ...
    var LaxElement = function LaxElement(selector, laxInstance, domElement, transformsData) {
      ...
      _defineProperty(this, "update", function (driverValues, frame) {
        ...
        if (this.transforms['scrollY']['canvas']) {
          this.updateImage(Math.ceil(styles['canvas']))
        }
      ...
      _defineProperty(this, "calculateTransforms", function () {
        ...
        if (_options['canvas']) {
          ...
          var canvasContext = $(selector).get(0).getContext('2d'),
          canvasWidth = _options['canvas']['canvasWidth'],
          canvasHeight = _options['canvas']['canvasHeight'];

          var currentFrame = function(index) {
            return _options['canvas']['canvasImagesPath']+''+index+''+_options['canvas']['canvasImagesFormat']
          }
          var preloadImages = function() {
              for (var i = 1; i <= _options['canvas']['frameCount']; i++) {
                var img = new Image();
          ...
          newImg.onload = function() {
              canvasContext.drawImage(newImg, 0, 0, canvasWidth, canvasHeight);
          }
